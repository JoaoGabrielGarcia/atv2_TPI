...
interface ProdutosPageState {
  produtos: Produto[];
  modalOpen: boolean;
  filtro: string;
}

class ProdutosPage extends React.Component<{}, ProdutosPageState> {
  constructor(props: {}) {
    super(props);
    this.state = {
  ...
  filtrarProdutos(produtos: Produto[], filtro: string) {
    switch (filtro) {
      case 'top10':
  ...
  handleSaveProduto = (produto: Produto) => {
  ...
  render() {
    const { produtos, modalOpen, filtro } = this.state;
